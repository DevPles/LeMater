...
AS $function$
  ...
  all_alerts AS (
    ...
    -- Vacinas recomendadas (SBIm extras): severidade 'info', dentro da janela
    SELECT
      'vac_extra_' || vse.id::text,
      'vacina'::text,
      'info'::text,
      ('Vacina recomendada disponível: ' || vse.vacina)::text,
      COALESCE(vse.mensagem, 'Vacina recomendada na sua janela gestacional.') ||
        CASE WHEN vse.fonte IS NOT NULL THEN ' (' || vse.fonte || ')' ELSE '' END,
      CURRENT_DATE
    FROM vaccine_schedule_extra vse
    WHERE COALESCE(
            (SELECT MAX(semana_gestacional) FROM clinical_measurements
             WHERE gestante_id = _gestante_id AND semana_gestacional IS NOT NULL),
            0
          ) BETWEEN vse.semana_min AND vse.semana_max
      AND NOT EXISTS (
        SELECT 1 FROM vaccinations v
        WHERE v.gestante_id = _gestante_id AND v.vacina = vse.vacina
      )
  )
