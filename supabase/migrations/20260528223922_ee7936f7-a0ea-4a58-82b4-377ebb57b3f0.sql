...
-- Atualiza handle_new_user para capturar os novos campos
...
BEGIN
  ...
  VALUES (
    ...
    NULLIF(NEW.raw_user_meta_data->>'altura_cm', '')::numeric,
    NULLIF(NEW.raw_user_meta_data->>'peso_inicial_kg', '')::numeric
  )
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    altura_cm = COALESCE(EXCLUDED.altura_cm, public.profiles.altura_cm),
    peso_inicial_kg = COALESCE(EXCLUDED.peso_inicial_kg, public.profiles.peso_inicial_kg),
    updated_at = now();
