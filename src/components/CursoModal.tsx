  ...
  useEffect(() => {
    fn({ data: { slug } })
      .then((d) => {
        ...
        // Auto seleciona primeira aula acessível
  ...
  return (
    <div onClick={fechar} style={overlay}>
      <div onClick={(e) => e.stopPropagation()} style={modal}>
        ...
        {data && (
          <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", minHeight: "82vh", maxHeight: "92vh" }}>
            {/* Sidebar */}
            <aside style={{ background: c.warm, borderRight: `1px solid ${c.border}`, overflow: "auto", padding: 24 }}>
              ...
              <h2 style={{ fontFamily: serif, fontSize: 26, fontWeight: 400, margin: "0 0 8px", lineHeight: 1.2 }}>{data.titulo}</h2>
                        ...
                        <li key={a.id}>
                          ...
                          >
                            ...
                            <span style={{ fontSize: 10, opacity: 0.7 }}>
                              {a.bloqueada ? "🔒" : a.previa_gratis ? "PRÉVIA" : `${a.duracao_min}min`}
                            </span>
                          </button>
                        </li>
                      );
            ...
            <section style={{ background: c.cream, overflow: "auto", padding: 32 }}>
              {!aulaSel && data.matriculado && (
                <div style={{ color: c.muted, padding: 40, textAlign: "center" }}>Selecione uma aula ao lado.</div>
              )}
              {!aulaSel && !data.matriculado && (
                <div>
                  {data.capa_url && <img src={data.capa_url} alt="" style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", border: `1px solid ${c.border}`, marginBottom: 24 }} />}
                  <h3 style={{ fontFamily: serif, fontSize: 28, fontWeight: 400 }}>Sobre o curso</h3>
                  {data.descricao_longa && <div style={{ fontSize: 15, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{data.descricao_longa}</div>}
                  {data.instrutor_nome && (
                    <div style={{ marginTop: 32, paddingTop: 24, borderTop: `1px solid ${c.border}` }}>
                      <div style={{ fontSize: 11, letterSpacing: "0.18em", color: c.muted, marginBottom: 8 }}>INSTRUTOR</div>
                      <h4 style={{ fontFamily: serif, fontSize: 22, margin: 0 }}>{data.instrutor_nome}</h4>
                      {data.instrutor_bio && <p style={{ marginTop: 8, color: c.muted, lineHeight: 1.6 }}>{data.instrutor_bio}</p>}
                    </div>
                  )}
                </div>
              )}
                ...
                <>
                  ...
                  {player && (
                    <>
                      ...
                      <h3 style={{ fontFamily: serif, fontSize: 28, fontWeight: 400, margin: "0 0 16px" }}>{player.aula.titulo}</h3>
                      ...
                      {player.conteudo.kind === "vazio" && (
                        <p style={{ color: c.muted }}>Conteúdo ainda não disponível.</p>
                      )}
...
const overlay: CSSProperties = { position: "fixed", inset: 0, background: "rgba(28,28,26,0.75)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 };
const modal: CSSProperties = { background: c.cream, width: "100%", maxWidth: 1200, maxHeight: "94vh", border: `1px solid ${c.border}`, position: "relative", overflow: "hidden" };
const closeBtn: CSSProperties = { position: "absolute", top: 10, right: 14, background: "rgba(255,255,255,0.85)", border: `1px solid ${c.border}`, width: 32, height: 32, borderRadius: "50%", cursor: "pointer", fontSize: 20, color: c.ink, zIndex: 10, fontFamily: sans };
